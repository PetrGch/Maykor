...
$arResult['sUrlPath'] = '/about/medialibrary/'.$arResult['NavTitle'].'/';
$arResult["NavQueryString"] = preg_replace("/PAGE=[a-z_]+(&amp;)?/", "", $arResult["NavQueryString"]);
	...
	else
	{
		...
		do
		{
			if ($arResult["nStartPage"] <= 2 || $arResult["nEndPage"]-$arResult["nStartPage"] <= 1 || abs($arResult['nStartPage']-$arResult["NavPageNomer"])<=2)
			{
				...
				<?else:?>
				<a class="adm-nav-page" href="<?=$arResult["sUrlPath"]?><?=$strNavQueryString?>PAGEN_<?=$arResult["NavNum"]?>=<?=$arResult["nStartPage"]?>">
					<?=$arResult["nStartPage"]?>
				</a>
				<?endif;
				$bFirst = false;
