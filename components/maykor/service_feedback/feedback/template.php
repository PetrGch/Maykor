...
foreach(array("SERVICE_HIDDEN", "EMAIL_ADMIN_HIDDEN") as $hidden_field){
...
<div class="page-header gy">
	<?//=$arResult["FORM_TITLE"]?> "Сделать запрос"
</div>
Свяжитесь с компанией, заполнив форму
<?
/***********************************************************************************
						form questions
***********************************************************************************/
...
//echo '<pre>'; print_r($arQuestion); echo '<pre>';
	...
	<?if(!empty($arFile)):?>
			<div style="float: right" class="m10 w100">
				...
				<span style="line-height: 30px; float: right"><?=$arFile["CAPTION"]?><?if ($arFile["REQUIRED"] == "Y"):?><?=$arResult["REQUIRED_SIGN"];?><?endif;?></span>
			</div>
	<?endif;?>
	...
	if (!empty($arAREA['key'])):?>
		...
		<?endif;?>
			...
			<div style="float: left; margin-left: 15px;"><?=$arQuestion["CAPTION"]?><?if ($arQuestion["REQUIRED"] == "Y"):?><?=$arResult["REQUIRED_SIGN"];?></div><?endif;?>
			<?=$arQuestion["HTML_CODE"]?>
	<?endif;?>
