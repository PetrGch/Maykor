...
if (is_array($arParams['HIDDEN_FIELD'])) {
...
//echo '<pre>'; print_r($arQuestion); echo '<pre>';
	...
	<?if (!empty($arFile)):?>
		...
		<div style="float: right" class="m10 w100">
			<span class="file" style="float: right">
			...
			<span class="fileLbl" style="line-height: 30px; float: right"><?=$arFile["CAPTION"]?><?if ($arFile["REQUIRED"] == "Y"):?><?=$arResult["REQUIRED_SIGN"];?><?endif;?></span>
		</div>
...
?>
			...
			<? endif; */ ?>
			<span class="req"><?=$arResult["REQUIRED_SIGN"];?> - <?=GetMessage("FORM_REQUIRED_FIELDS")?>	
<?
} // isUseCaptcha
?>
</div>
...
//ввод ссылки и заголовка страницы отправки
	if($('input[data-name="PAGE_TITLE"]')[0]){
		$('input[data-name="PAGE_TITLE"]').val(document.title);
	}
	
	if($('input[data-name="PAGE_LINK"]')[0]){
		$('input[data-name="PAGE_LINK"]').val(location.href);
	}
</script>
