...
$(document).ready(function() {
	...
	if (getInternetExplorerVersion() >= 10){
		$('.ach-img').each(function(){
			var el = $(this);
			el.css({"position":"relative"}).wrap("<div class='img_wrapper' style='display: block; margin: 0 auto;'>").clone().addClass('img_grayscale').css({"position":"absolute","z-index":"5","opacity":"0"}).insertBefore(el).queue(function(){
				var el = $(this);
		...
		$('.ach-img').hover(
			function () {
				$(this).parent().find('.ach-img:first').stop().animate({opacity:1}, 200);
			}, 
			function () {
...
function initSomeElements() {
	...
	//формы отзыва для закупки
	$("input.purchase_feedback").on("click", function(e) {
		var purchase = $(this).attr('data-purchase');
		$( ".popup .popup-content" ).load( "/about/zakupki/procedure/feedback.php", {'PURCHASE': purchase}, function(){
			$(".feedback").trigger("click");
			initDDs(".feedback-form.big");
			initPMask();
		});
...
function byHashUrl(){
	...
	if(location.hash != '' && yandex.test(location.hash) == 0){
		...
		var find_zakupki_info = /\/about\/zakupki\/info\//;
		...
		if (find_media.test(page) != 0){
			...
			else if (find_zakupki_info.test(path) != 0){
				path = '/about/zakupki/info/';
			}
